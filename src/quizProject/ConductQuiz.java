...
class Question {
    ...
    void displayAudiencePoll() {
        ...
        for (int i = 0; i < 4; i++) {
            System.out.println("Option " + (i + 1) + ": " + options[i] + " - " + percentages[i] + "%");
        }
    }

    void display50_50Lifeline(Scanner sc, Candidate candidate, int[] prizeAmount) {
        System.out.println("You chose the 50-50 lifeline.");
        String incorrectOption = selectIncorrectOption();

        String[] selectedOptions = {correctOption, incorrectOption};
        shuffleOptions(selectedOptions);
        ...
        String chosenAnswer = selectedOptions[chosenOption - 1];

        if (chosenAnswer.equals(correctOption)) {
            candidate.displayCongrats();
            System.out.println("You've won: " + prizeAmount[0]);
        } else {
            candidate.displayEncouragement();
            System.out.println("You lost! Total prize: " + prizeAmount[0]);
            System.exit(0);
        }
    }
    
    private String selectIncorrectOption() {
        String incorrectOption = null;
        while (incorrectOption == null || incorrectOption.equals(correctOption)) {
            int randomIndex = (int) (Math.random() * 4);
            if (!options[randomIndex].equals(correctOption)) {
    ...
    private void shuffleOptions(String[] selectedOptions) {
        if (Math.random() < 0.5) {
...
public class ConductQuiz {
    ...
    public static void main(String[] args) {
        ...
        for (Question question : questions) {
            ...
            displayOptions(question, maxOption);
            ...
            if (selectedOption.equals("Lifeline") && lifelineUses < MAX_LIFELINE_USES) {
                lifelineUses = handleLifeline(sc,candidate, question, lifelineUses);
                continue;
            }
    ...
    static void displayRules() {
        ...
        System.out.println();
        System.out.println("(Questions are collected from - https://blog.livereacting.com/100-fun-general-knowledge-quiz-questions/)");
    }
    
    static void displayOptions(Question question, int maxOption) {
        for (int i = 0; i < maxOption; i++) {
            System.out.println((i + 1) + " - " + question.options[i]);
        }
    }

    static Question[] initializeQuestions() {
        return new Question[]{
				new Question(
    ...
    static int promptForValidOption(Scanner sc, int minOption, int maxOption) {
        ...
        while (true) {
        	if(minOption == maxOption) {
        		System.out.print("Enter your option number ("+minOption+"): ");
        	}else {        		
    ...
    static int handleLifeline(Scanner sc, Candidate candidate, Question question, int lifelineUses) {
        ...
        if (canChoose50_50) System.out.println("1: 50-50");
        if (canChooseAudiencePoll) System.out.println("2: Audience Poll");

//        int lifelineChoice = promptForValidOption(sc, 1, canChooseAudiencePoll ? 2 : 1);
        
        int lifelineChoice = 0;
        
        if(canChoose50_50 == true && canChooseAudiencePoll == true) {
        	lifelineChoice = promptForValidOption(sc, 1, 2);
        }else if (canChoose50_50 == false && canChooseAudiencePoll == true) {
        	lifelineChoice = promptForValidOption(sc, 2, 2);
        }else if (canChoose50_50 == true && canChooseAudiencePoll == false) {
        	lifelineChoice = promptForValidOption(sc, 1, 1);
        }
        ...
        if (lifelineChoice == 1 && canChoose50_50) {
            ...
            System.out.println("Please choose your option after the audience poll.");
            int chosenOption = promptForValidOption(sc, 1, 4);
            String selectedOption = question.options[chosenOption - 1];

            if (!question.isAnswerCorrect(selectedOption, candidate)) {
                System.out.println("You lost! Total prize: " + question.prizeAmount);
                return lifelineUses; // Do not exit; continue gracefully
            }
        } else {
            System.out.println("Invalid lifeline choice. Returning to the question.");
        }

        return lifelineUses + 1;
    }
